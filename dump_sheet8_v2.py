...
def dump_all_labels_v2(template_path, sheet_rel_path):
    print(f"Dumping ALL labels (v2) from: {sheet_rel_path}")
    shared_strings = []
    ...
    for row in sheet_data.findall(f'{{{ns}}}row'):
        for cell in row.findall(f'{{{ns}}}c'):
            ...
            if cell.get('t') == 's':
                ...
                tis = cell.findall(f'.//{{{ns}}}t')
                val = "".join(t.text or "" for t in tis).strip()
...
if __name__ == "__main__":
    ...
    dump_all_labels_v2(template, "worksheets/sheet8.xml")
