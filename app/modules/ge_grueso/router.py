"""
Router for GE Grueso - ASTM C127-25.
"""
...
from .excel import TEMPLATE_FILENAME, generate_ge_grueso_excel
from .models import GeGruesoEnsayo
from .schemas import (
    GeGruesoDetalleResponse,
    GeGruesoEnsayoResponse,
    GeGruesoRequest,
    GeGruesoSaveResponse,
)

router = APIRouter(prefix="/api/ge-grueso", tags=["Laboratorio GE Grueso"])
logger = logging.getLogger(__name__)
_PAYLOAD_COLUMN_READY = False
BUCKET_NAME = "ge-grueso"
...
def _upload_to_supabase_storage(file_bytes: bytes, bucket: str, object_path: str) -> str | None:
    ...
    if not supabase_url or not supabase_key:
        logger.warning("Supabase URL/Key no configurado. Se omite subida de GE Grueso.")
        return None
    ...
    try:
        ...
        logger.error("Storage upload GE Grueso failed: %s - %s", resp.status_code, resp.text)
        return None
    except Exception as exc:
        logger.error("Error subiendo GE Grueso a storage: %s", exc)
        return None
...
def _delete_from_supabase_storage(bucket: str, object_path: str) -> bool:
    ...
    try:
        ...
        if resp.status_code in (200, 204):
            return True
        logger.warning(
            "No se pudo eliminar objeto previo de GE Grueso: %s/%s (%s)",
            bucket,
            object_path,
            resp.status_code,
        )
        ...
        logger.warning("Error eliminando objeto previo de GE Grueso: %s", exc)
        return False
...
def _move_to_supabase_trash(bucket: str, object_path: str) -> str | None:
    ...
    try:
        ...
        logger.warning("Error moviendo GE Grueso a trash con move API: %s", exc)
    ...
    try:
        ...
        logger.warning("Error en fallback copy+delete para trash GE Grueso: %s", exc)
        return None
...
def _ensure_payload_column(db: Session) -> None:
    ...
    db.execute(text("ALTER TABLE ge_grueso_ensayos ADD COLUMN IF NOT EXISTS payload_json JSON"))
    db.execute(text("ALTER TABLE ge_grueso_ensayos ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ"))
    db.execute(text("ALTER TABLE ge_grueso_ensayos ADD COLUMN IF NOT EXISTS masa_muestra_inicial_total_kg DOUBLE PRECISION"))
    db.flush()
...
def _is_payload_completo(payload: GeGruesoRequest) -> bool:
    ...
    if not _is_selected(payload.seco_horno_110_si_no):
        return False

    if payload.fr1_a_g is None or payload.fr1_b_g is None or payload.fr1_c_g is None or payload.fr1_d_g is None:
        return False
    if payload.fr2_a_g is None or payload.fr2_b_g is None or payload.fr2_c_g is None or payload.fr2_d_g is None:
        return False
    return payload.masa_muestra_inicial_total_kg is not None


def _build_numero_ensayo(payload: GeGruesoRequest) -> str:
...
def _guardar_ensayo(
    ...
    if ensayo_id is not None:
        ensayo = (
            db.query(GeGruesoEnsayo)
            .filter(
                GeGruesoEnsayo.id == ensayo_id,
                GeGruesoEnsayo.deleted_at.is_(None),
            )
            .first()
        )
        if not ensayo:
            raise HTTPException(status_code=404, detail="Ensayo GE Grueso no encontrado para edicion.")
        old_bucket = ensayo.bucket
        old_object_key = ensayo.object_key
    else:
        ensayo = GeGruesoEnsayo()
        db.add(ensayo)
    ...
    ensayo.masa_muestra_inicial_total_kg = payload.masa_muestra_inicial_total_kg
...
def _to_detalle_response(ensayo: GeGruesoEnsayo) -> GeGruesoDetalleResponse:
    payload = None
    if ensayo.payload_json:
        try:
            payload = GeGruesoRequest.model_validate(ensayo.payload_json)
        except Exception:
            logger.warning("payload_json invalido en ge_grueso_ensayos.id=%s", ensayo.id, exc_info=True)

    return GeGruesoDetalleResponse(
        id=ensayo.id,
        ...
        masa_muestra_inicial_total_kg=ensayo.masa_muestra_inicial_total_kg,
...
@router.get("/", response_model=list[GeGruesoEnsayoResponse])
async def listar_ensayos_ge_grueso(
    skip: int = 0,
    limit: int = 1000,
    ...
    return (
        db.query(GeGruesoEnsayo)
        .filter(GeGruesoEnsayo.deleted_at.is_(None))
        .order_by(desc(GeGruesoEnsayo.fecha_creacion))
        .offset(skip)
        .limit(limit)
        .all()
    )
...
@router.get("/{ensayo_id}", response_model=GeGruesoDetalleResponse)
async def obtener_ensayo_ge_grueso(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    _ensure_payload_column(db)
    ...
    if not ensayo:
        raise HTTPException(status_code=404, detail="Ensayo GE Grueso no encontrado.")
    return _to_detalle_response(ensayo)


@router.delete("/{ensayo_id}")
async def eliminar_ensayo_ge_grueso(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    _ensure_payload_column(db)
    ...
    try:
        ...
        db.rollback()
        logger.exception("Error enviando ensayo GE Grueso a papelera id=%s", ensayo_id)
        raise HTTPException(status_code=500, detail="No se pudo enviar el ensayo GE Grueso a papelera.")

    return {
        "message": "Ensayo GE Grueso movido a papelera correctamente",
        "id": ensayo_id,
...
def generar_excel_ge_grueso(
    ...
    try:
        ...
        payload_obj = resolve_export_payload(
            ...
            ensayo_id=ensayo_id,
            db=db,
            model=GeGruesoEnsayo,
            request_model=GeGruesoRequest,
            display_name="GE Grueso",
        )

        _apply_footer_defaults(payload_obj)
        excel_bytes = generate_ge_grueso_excel(payload_obj, db=db)

        today = date.today()
        filename = build_formato_filename(payload_obj.muestra, "AG28", "GE GRUESO", template_filename=TEMPLATE_FILENAME)

        safe_ot = _safe_filename(payload_obj.numero_ot, extension="")
        safe_muestra = _safe_filename(payload_obj.muestra, extension="")
        storage_name = f"GE_GRUESO_{safe_ot}_{safe_muestra}_{today.strftime('%Y%m%d')}.xlsx"
        ...
        if not download:
            return GeGruesoSaveResponse(
                ...
                masa_muestra_inicial_total_kg=ensayo_guardado.masa_muestra_inicial_total_kg,
        ...
        headers = {
            ...
            "X-Ge-Grueso-Id": str(ensayo_guardado.id),
        }
        if storage_object_key:
...
def descargar_excel_ge_grueso(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    return generar_excel_ge_grueso(payload=None, download=True, ensayo_id=ensayo_id, db=db)
