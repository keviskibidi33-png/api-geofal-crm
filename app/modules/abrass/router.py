"""
Router for ABRASS - ASTM C131/C131M-20.
"""
...
from .schemas import (
    ...
    AbrassEnsayoResponse,
    AbrassRequest,
    AbrassSaveResponse,
)

router = APIRouter(prefix="/api/abrass", tags=["Laboratorio ABRASS"])
...
def _upload_to_supabase_storage(file_bytes: bytes, bucket: str, object_path: str) -> str | None:
    ...
    if not supabase_url or not supabase_key:
        logger.warning("Supabase URL/Key no configurado. Se omite subida de ABRASS.")
        return None
    ...
    try:
        ...
        logger.error("Storage upload ABRASS failed: %s - %s", resp.status_code, resp.text)
        return None
    except Exception as exc:
        logger.error("Error subiendo ABRASS a storage: %s", exc)
        return None
...
def _delete_from_supabase_storage(bucket: str, object_path: str) -> bool:
    ...
    try:
        ...
        if resp.status_code in (200, 204):
            return True
        logger.warning(
            "No se pudo eliminar objeto previo de ABRASS: %s/%s (%s)",
            bucket,
            object_path,
            resp.status_code,
        )
        ...
        logger.warning("Error eliminando objeto previo de ABRASS: %s", exc)
        return False
...
def _move_to_supabase_trash(bucket: str, object_path: str) -> str | None:
    ...
    try:
        ...
        logger.warning("Error moviendo ABRASS a trash con move API: %s", exc)
    ...
    try:
        ...
        logger.warning("Error en fallback copy+delete para trash ABRASS: %s", exc)
        return None
...
def _ensure_payload_column(db: Session) -> None:
    ...
    db.execute(text("ALTER TABLE abrass_ensayos ADD COLUMN IF NOT EXISTS payload_json JSON"))
    db.execute(text("ALTER TABLE abrass_ensayos ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ"))
    db.execute(
        text(
            "ALTER TABLE abrass_ensayos "
            "ADD COLUMN IF NOT EXISTS perdida_abrasion_promedio_pct DOUBLE PRECISION"
        )
    )
    db.flush()
...
def _is_payload_completo(payload: AbrassRequest) -> bool:
    ...
    main_numbers = [
        payload.masa_muestra_inicial_g,
        payload.masa_muestra_inicial_seca_despues_lavado_g,
        payload.masa_muestra_inicial_seca_constante_despues_lavado_g,
    ]
    if not all(value is not None for value in main_numbers):
        return False

    has_tamiz = any(
        value is not None
        for value in (
            payload.gradacion_a_tamiz_g
            + payload.gradacion_b_tamiz_g
            + payload.gradacion_c_tamiz_g
            + payload.gradacion_d_tamiz_g
        )
    )
    return has_tamiz


def _build_numero_ensayo(payload: AbrassRequest) -> str:
    return f"{payload.numero_ot}-ABRASS"
...
def _apply_footer_defaults(payload: AbrassRequest) -> None:
...
def _guardar_ensayo(
    ...
    if ensayo_id is not None:
        ensayo = (
            db.query(AbrassEnsayo)
            .filter(
                AbrassEnsayo.id == ensayo_id,
                AbrassEnsayo.deleted_at.is_(None),
            )
            .first()
        )
        if not ensayo:
            raise HTTPException(status_code=404, detail="Ensayo ABRASS no encontrado para edicion.")
    ...
    ensayo.perdida_abrasion_promedio_pct = payload.perdida_abrasion_promedio()
...
def _to_detalle_response(ensayo: AbrassEnsayo) -> AbrassDetalleResponse:
    payload = None
    if ensayo.payload_json:
        try:
            payload = AbrassRequest.model_validate(ensayo.payload_json)
        except Exception:
            logger.warning("payload_json invalido en abrass_ensayos.id=%s", ensayo.id, exc_info=True)

    return AbrassDetalleResponse(
        id=ensayo.id,
        ...
        perdida_abrasion_promedio_pct=ensayo.perdida_abrasion_promedio_pct,
...
async def listar_ensayos_abrass(
    ...
    return (
        db.query(AbrassEnsayo)
        .filter(AbrassEnsayo.deleted_at.is_(None))
        .order_by(desc(AbrassEnsayo.fecha_creacion))
        .offset(skip)
        .limit(limit)
        .all()
    )
...
@router.get("/{ensayo_id}", response_model=AbrassDetalleResponse)
async def obtener_ensayo_abrass(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    _ensure_payload_column(db)
    ...
    if not ensayo:
        raise HTTPException(status_code=404, detail="Ensayo ABRASS no encontrado.")
    return _to_detalle_response(ensayo)


@router.delete("/{ensayo_id}")
async def eliminar_ensayo_abrass(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    _ensure_payload_column(db)
    ...
    try:
        ...
        db.rollback()
        logger.exception("Error enviando ensayo ABRASS a papelera id=%s", ensayo_id)
        raise HTTPException(status_code=500, detail="No se pudo enviar el ensayo ABRASS a papelera.")

    return {
        "message": "Ensayo ABRASS movido a papelera correctamente",
        "id": ensayo_id,
...
def generar_excel_abrass(
    ...
    try:
        ...
        payload_obj = resolve_export_payload(
            ...
            db=db,
            model=AbrassEnsayo,
            request_model=AbrassRequest,
            display_name="ABRASS",
        )

        _apply_footer_defaults(payload_obj)
        excel_bytes = generate_abrass_excel(payload_obj, db=db)

        today = date.today()
        filename = build_formato_filename(payload_obj.muestra, "AG36", "ABRASS", template_filename=TEMPLATE_FILENAME)

        safe_ot = _safe_filename(payload_obj.numero_ot, extension="")
        safe_muestra = _safe_filename(payload_obj.muestra, extension="")
        storage_name = f"ABRASS_{safe_ot}_{safe_muestra}_{today.strftime('%Y%m%d')}.xlsx"
        ...
        if not download:
            return AbrassSaveResponse(
                ...
                perdida_abrasion_promedio_pct=ensayo_guardado.perdida_abrasion_promedio_pct,
        ...
        headers = {
            ...
            "X-ABRASS-Id": str(ensayo_guardado.id),
        }
        if storage_object_key:
        ...
        db.rollback()
        logger.exception("Error inesperado en generar_excel_abrass")
        raise HTTPException(status_code=500, detail=f"Error generando Excel ABRASS: {str(exc)}")


@router.get("/{ensayo_id}/excel")
def descargar_excel_abrass(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    return generar_excel_abrass(payload=None, download=True, ensayo_id=ensayo_id, db=db)
