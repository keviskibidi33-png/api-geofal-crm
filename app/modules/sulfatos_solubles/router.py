...
router = APIRouter(prefix="/api/sulfatos-solubles", tags=["Laboratorio Sulfatos Solubles"])
logger = logging.getLogger(__name__)
_PAYLOAD_COLUMN_READY = False
BUCKET_NAME = "sulfatos-solubles"
...
def _upload_to_supabase_storage(file_bytes: bytes, bucket: str, object_path: str) -> str | None:
    ...
    if not supabase_url or not supabase_key:
        logger.warning("Supabase URL/Key missing. Upload skipped for Sulfatos Solubles.")
        return None
    ...
    try:
        ...
        logger.error("Storage upload Sulfatos Solubles failed: %s - %s", resp.status_code, resp.text)
        return None
    except Exception as exc:
        logger.error("Error uploading Sulfatos Solubles to storage: %s", exc)
        return None
...
def _delete_from_supabase_storage(bucket: str, object_path: str) -> bool:
    ...
    try:
        ...
        if resp.status_code in (200, 204):
            return True
        logger.warning(
            "Unable to delete Sulfatos Solubles object: %s/%s (%s)",
            bucket,
            object_path,
            resp.status_code,
        )
        ...
        logger.warning("Error deleting Sulfatos Solubles object: %s", exc)
        return False
...
def _move_to_supabase_trash(bucket: str, object_path: str) -> str | None:
    ...
    try:
        ...
        logger.warning("Error moving Sulfatos Solubles to trash via move API: %s", exc)
    ...
    try:
        ...
        logger.warning("Error in fallback copy+delete for Sulfatos Solubles trash: %s", exc)
        return None
...
def _ensure_payload_column(db: Session) -> None:
    ...
    db.execute(text("ALTER TABLE sulfatos_solubles_ensayos ADD COLUMN IF NOT EXISTS payload_json JSON"))
    db.execute(text("ALTER TABLE sulfatos_solubles_ensayos ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ"))
    db.flush()
...
def _is_payload_completo(payload: SulfatosSolublesRequest) -> bool:
...
def _build_numero_ensayo(payload: SulfatosSolublesRequest) -> str:
    return f"{payload.numero_ot}-SULF"
...
def _apply_footer_defaults(payload: SulfatosSolublesRequest) -> None:
...
def _guardar_ensayo(
    db: Session,
    payload: SulfatosSolublesRequest,
    storage_object_key: str | None,
    ensayo_id: int | None,
    estado: str,
) -> SulfatosSolublesEnsayo:
    ...
    if ensayo_id is not None:
        ensayo = (
            db.query(SulfatosSolublesEnsayo)
            .filter(
                SulfatosSolublesEnsayo.id == ensayo_id,
                SulfatosSolublesEnsayo.deleted_at.is_(None),
            )
            .first()
        )
        if not ensayo:
            raise HTTPException(status_code=404, detail="Ensayo Sulfatos Solubles no encontrado para edicion.")
        old_bucket = ensayo.bucket
        old_object_key = ensayo.object_key
    else:
        ensayo = SulfatosSolublesEnsayo()
        db.add(ensayo)
...
def _to_detalle_response(ensayo: SulfatosSolublesEnsayo) -> SulfatosSolublesDetalleResponse:
    payload = None
    if ensayo.payload_json:
        try:
            payload = SulfatosSolublesRequest.model_validate(ensayo.payload_json)
        except Exception:
            logger.warning("payload_json invalid in sulfatos_solubles_ensayos.id=%s", ensayo.id, exc_info=True)

    return SulfatosSolublesDetalleResponse(
        id=ensayo.id,
...
@router.get("/", response_model=list[SulfatosSolublesEnsayoResponse])
async def listar_ensayos(
    ...
    return (
        db.query(SulfatosSolublesEnsayo)
        .filter(SulfatosSolublesEnsayo.deleted_at.is_(None))
        .order_by(desc(SulfatosSolublesEnsayo.fecha_creacion))
        .offset(skip)
        .limit(limit)
        .all()
    )
...
@router.get("/{ensayo_id}", response_model=SulfatosSolublesDetalleResponse)
async def obtener_ensayo(
    ...
    if not ensayo:
        raise HTTPException(status_code=404, detail="Ensayo Sulfatos Solubles no encontrado.")
...
async def eliminar_ensayo(
    ...
    try:
        ...
        db.rollback()
        logger.exception("Error moving Sulfatos Solubles ensayo to trash id=%s", ensayo_id)
        raise HTTPException(status_code=500, detail="Unable to move Sulfatos Solubles ensayo to trash.")

    return {
        "message": "Ensayo Sulfatos Solubles moved to trash",
        "id": ensayo_id,
...
def generar_excel(
    ...
    try:
        ...
        payload_obj = resolve_export_payload(
            ...
            ensayo_id=ensayo_id,
            db=db,
            model=SulfatosSolublesEnsayo,
            request_model=SulfatosSolublesRequest,
            display_name="Sulfatos Solubles",
        )

        _apply_footer_defaults(payload_obj)
        excel_bytes = generate_sulfatos_solubles_excel(payload_obj)

        today = date.today()
        filename = build_formato_filename(payload_obj.muestra, "SU15", "SULFATOS SOLUBLES")

        safe_ot = _safe_filename(payload_obj.numero_ot, extension="")
        safe_muestra = _safe_filename(payload_obj.muestra, extension="")
        storage_name = f"SULF_{safe_ot}_{safe_muestra}_{today.strftime('%Y%m%d')}.xlsx"
        ...
        if not download:
            return SulfatosSolublesSaveResponse(
        ...
        headers = {
            ...
            "X-SULF-Id": str(ensayo_guardado.id),
        }
        if storage_object_key:
        ...
        logger.error("Template missing for Sulfatos Solubles: %s", exc)
        raise HTTPException(status_code=404, detail="Template not found: Template_SULFATOS_SOLUBLES.xlsx")
    except HTTPException:
        db.rollback()
        raise
    except Exception as exc:
        db.rollback()
        logger.exception("Excel export failed for Sulfatos Solubles")
        raise HTTPException(status_code=500, detail=f"Export failed: {exc}")


@router.get("/{ensayo_id}/excel")
def descargar_excel_sulfatos_solubles(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
