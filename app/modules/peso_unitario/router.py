"""
Router for Peso Unitario - ASTM C29/C29M-23.
"""
...
from .excel import TEMPLATE_FILENAME, generate_peso_unitario_excel
from .models import PesoUnitarioEnsayo
from .schemas import (
    PesoUnitarioDetalleResponse,
    PesoUnitarioEnsayoResponse,
    PesoUnitarioRequest,
    PesoUnitarioSaveResponse,
)

router = APIRouter(prefix="/api/peso-unitario", tags=["Laboratorio Peso Unitario"])
logger = logging.getLogger(__name__)
_PAYLOAD_COLUMN_READY = False
BUCKET_NAME = "peso-unitario"
...
def _upload_to_supabase_storage(file_bytes: bytes, bucket: str, object_path: str) -> str | None:
    ...
    if not supabase_url or not supabase_key:
        logger.warning("Supabase URL/Key no configurado. Se omite subida de Peso Unitario.")
        return None
    ...
    try:
        ...
        logger.error("Storage upload Peso Unitario failed: %s - %s", resp.status_code, resp.text)
        return None
    except Exception as exc:
        logger.error("Error subiendo Peso Unitario a storage: %s", exc)
        return None
...
def _delete_from_supabase_storage(bucket: str, object_path: str) -> bool:
    ...
    try:
        ...
        if resp.status_code in (200, 204):
            return True
        logger.warning(
            "No se pudo eliminar objeto previo de Peso Unitario: %s/%s (%s)",
            bucket,
            object_path,
            resp.status_code,
        )
        ...
        logger.warning("Error eliminando objeto previo de Peso Unitario: %s", exc)
        return False
...
def _move_to_supabase_trash(bucket: str, object_path: str) -> str | None:
    ...
    try:
        ...
        logger.warning("Error moviendo Peso Unitario a trash con move API: %s", exc)
    ...
    try:
        ...
        logger.warning("Error en fallback copy+delete para trash Peso Unitario: %s", exc)
        return None
...
def _ensure_payload_column(db: Session) -> None:
    ...
    db.execute(text("ALTER TABLE peso_unitario_ensayos ADD COLUMN IF NOT EXISTS payload_json JSON"))
    db.execute(text("ALTER TABLE peso_unitario_ensayos ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ"))
    db.execute(
        text(
            "ALTER TABLE peso_unitario_ensayos "
            "ADD COLUMN IF NOT EXISTS densidad_aparente_promedio_kg_m3 DOUBLE PRECISION"
        )
    )
    db.execute(
        text(
            "ALTER TABLE peso_unitario_ensayos "
            "ADD COLUMN IF NOT EXISTS vacios_promedio_pct DOUBLE PRECISION"
        )
    )
    db.flush()
...
def _is_payload_completo(payload: PesoUnitarioRequest) -> bool:
    ...
    if payload.recipiente_masa_medida_kg is None or payload.recipiente_volumen_m3 is None:
        return False

    if payload.metodo_compactacion in {None, "-", ""}:
        return False

    return payload.densidad_aparente_promedio_kg_m3 is not None and payload.vacios_promedio_pct is not None


def _build_numero_ensayo(payload: PesoUnitarioRequest) -> str:
    return f"{payload.numero_ot}-PESO-UNITARIO"
...
def _apply_footer_defaults(payload: PesoUnitarioRequest) -> None:
...
def _guardar_ensayo(
    ...
    if ensayo_id is not None:
        ensayo = (
            db.query(PesoUnitarioEnsayo)
            .filter(
                PesoUnitarioEnsayo.id == ensayo_id,
                PesoUnitarioEnsayo.deleted_at.is_(None),
            )
            .first()
        )
        if not ensayo:
            raise HTTPException(status_code=404, detail="Ensayo Peso Unitario no encontrado para edicion.")
        old_bucket = ensayo.bucket
        old_object_key = ensayo.object_key
    else:
        ensayo = PesoUnitarioEnsayo()
        db.add(ensayo)
    ...
    ensayo.densidad_aparente_promedio_kg_m3 = payload.densidad_aparente_promedio_kg_m3
    ensayo.vacios_promedio_pct = payload.vacios_promedio_pct
...
def _to_detalle_response(ensayo: PesoUnitarioEnsayo) -> PesoUnitarioDetalleResponse:
    payload = None
    ...
    return PesoUnitarioDetalleResponse(
        ...
        densidad_aparente_promedio_kg_m3=ensayo.densidad_aparente_promedio_kg_m3,
        vacios_promedio_pct=ensayo.vacios_promedio_pct,
...
@router.get("/", response_model=list[PesoUnitarioEnsayoResponse])
async def listar_ensayos_peso_unitario(
    skip: int = 0,
    limit: int = 1000,
    ...
    return (
        db.query(PesoUnitarioEnsayo)
        .filter(PesoUnitarioEnsayo.deleted_at.is_(None))
        .order_by(desc(PesoUnitarioEnsayo.fecha_creacion))
        .offset(skip)
        .limit(limit)
        .all()
    )
...
@router.get("/{ensayo_id}", response_model=PesoUnitarioDetalleResponse)
async def obtener_ensayo_peso_unitario(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    _ensure_payload_column(db)
    ...
    if not ensayo:
        raise HTTPException(status_code=404, detail="Ensayo Peso Unitario no encontrado.")
    return _to_detalle_response(ensayo)


@router.delete("/{ensayo_id}")
async def eliminar_ensayo_peso_unitario(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    _ensure_payload_column(db)
    ...
    try:
        ...
        db.rollback()
        logger.exception("Error enviando ensayo Peso Unitario a papelera id=%s", ensayo_id)
        raise HTTPException(status_code=500, detail="No se pudo enviar el ensayo Peso Unitario a papelera.")

    return {
        "message": "Ensayo Peso Unitario movido a papelera correctamente",
        "id": ensayo_id,
...
@router.post("/excel")
def generar_excel_peso_unitario(
    ...
    try:
        ...
        payload_obj = resolve_export_payload(
            ...
            ensayo_id=ensayo_id,
            db=db,
            model=PesoUnitarioEnsayo,
            request_model=PesoUnitarioRequest,
            display_name="Peso Unitario",
        )

        _apply_footer_defaults(payload_obj)
        excel_bytes = generate_peso_unitario_excel(payload_obj)

        today = date.today()
        filename = build_formato_filename(payload_obj.muestra, "AG22", "PESO UNITARIO", template_filename=TEMPLATE_FILENAME)

        safe_ot = _safe_filename(payload_obj.numero_ot, extension="")
        safe_muestra = _safe_filename(payload_obj.muestra, extension="")
        storage_name = f"PESO_UNITARIO_{safe_ot}_{safe_muestra}_{today.strftime('%Y%m%d')}.xlsx"
        ...
        if not download:
            return PesoUnitarioSaveResponse(
                ...
                densidad_aparente_promedio_kg_m3=ensayo_guardado.densidad_aparente_promedio_kg_m3,
                vacios_promedio_pct=ensayo_guardado.vacios_promedio_pct,
        ...
        headers = {
            ...
            "X-Peso-Unitario-Id": str(ensayo_guardado.id),
        }
        if storage_object_key:
        ...
        db.rollback()
        logger.exception("Error inesperado en generar_excel_peso_unitario")
        raise HTTPException(status_code=500, detail=f"Error generando Excel Peso Unitario: {str(exc)}")


@router.get("/{ensayo_id}/excel")
def descargar_excel_peso_unitario(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    return generar_excel_peso_unitario(payload=None, download=True, ensayo_id=ensayo_id, db=db)
