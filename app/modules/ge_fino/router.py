"""
Router for GE Fino - ASTM C128-25.
"""
...
from .schemas import (
    ...
    GeFinoEnsayoResponse,
    GeFinoRequest,
    GeFinoSaveResponse,
)

router = APIRouter(prefix="/api/ge-fino", tags=["Laboratorio GE Fino"])
...
def _upload_to_supabase_storage(file_bytes: bytes, bucket: str, object_path: str) -> str | None:
    ...
    if not supabase_url or not supabase_key:
        logger.warning("Supabase URL/Key no configurado. Se omite subida de GE Fino.")
        return None
    ...
    try:
        ...
        logger.error("Storage upload GE Fino failed: %s - %s", resp.status_code, resp.text)
        return None
    except Exception as exc:
        logger.error("Error subiendo GE Fino a storage: %s", exc)
        return None
...
def _delete_from_supabase_storage(bucket: str, object_path: str) -> bool:
    ...
    try:
        ...
        if resp.status_code in (200, 204):
            return True
        logger.warning(
            "No se pudo eliminar objeto previo de GE Fino: %s/%s (%s)",
            bucket,
            object_path,
            resp.status_code,
        )
        ...
        logger.warning("Error eliminando objeto previo de GE Fino: %s", exc)
        return False
...
def _move_to_supabase_trash(bucket: str, object_path: str) -> str | None:
    ...
    try:
        ...
        logger.warning("Error moviendo GE Fino a trash con move API: %s", exc)
    ...
    try:
        ...
        logger.warning("Error en fallback copy+delete para trash GE Fino: %s", exc)
        return None
...
def _is_payload_completo(payload: GeFinoRequest) -> bool:
    ...
    if payload.valor_s_g is None or payload.valor_c_g is None or payload.valor_b_g is None:
        return False
    if payload.valor_a_g is None:
        return False
    return payload.absorcion_pct is not None


def _build_numero_ensayo(payload: GeFinoRequest) -> str:
    return f"{payload.numero_ot}-GE-FINO"
...
def _apply_footer_defaults(payload: GeFinoRequest) -> None:
...
def _guardar_ensayo(
    ...
    if ensayo_id is not None:
        ensayo = (
            db.query(GeFinoEnsayo)
            .filter(
                GeFinoEnsayo.id == ensayo_id,
                GeFinoEnsayo.deleted_at.is_(None),
            )
            .first()
        )
        if not ensayo:
            raise HTTPException(status_code=404, detail="Ensayo GE Fino no encontrado para edicion.")
    ...
    ensayo.absorcion_pct = payload.absorcion_pct
...
def _to_detalle_response(ensayo: GeFinoEnsayo) -> GeFinoDetalleResponse:
    payload = None
...
@router.get("/", response_model=list[GeFinoEnsayoResponse])
async def listar_ensayos_ge_fino(
    skip: int = 0,
    limit: int = 1000,
    ...
    return (
        db.query(GeFinoEnsayo)
        .filter(GeFinoEnsayo.deleted_at.is_(None))
        .order_by(desc(GeFinoEnsayo.fecha_creacion))
        .offset(skip)
        .limit(limit)
        .all()
    )
...
@router.get("/{ensayo_id}", response_model=GeFinoDetalleResponse)
async def obtener_ensayo_ge_fino(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    _ensure_payload_column(db)
    ...
    if not ensayo:
        raise HTTPException(status_code=404, detail="Ensayo GE Fino no encontrado.")
    return _to_detalle_response(ensayo)


@router.delete("/{ensayo_id}")
async def eliminar_ensayo_ge_fino(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    _ensure_payload_column(db)
    ...
    try:
        ...
        db.rollback()
        logger.exception("Error enviando ensayo GE Fino a papelera id=%s", ensayo_id)
        raise HTTPException(status_code=500, detail="No se pudo enviar el ensayo GE Fino a papelera.")

    return {
        "message": "Ensayo GE Fino movido a papelera correctamente",
        "id": ensayo_id,
...
def generar_excel_ge_fino(
    ...
    try:
        ...
        payload_obj = resolve_export_payload(
            ...
            db=db,
            model=GeFinoEnsayo,
            request_model=GeFinoRequest,
            display_name="GE Fino",
        )

        _apply_footer_defaults(payload_obj)
        excel_bytes = generate_ge_fino_excel(payload_obj)

        today = date.today()
        filename = build_formato_filename(payload_obj.muestra, "AG18", "GE FINO", template_filename=TEMPLATE_FILENAME)

        safe_ot = _safe_filename(payload_obj.numero_ot, extension="")
        safe_muestra = _safe_filename(payload_obj.muestra, extension="")
        storage_name = f"GE_FINO_{safe_ot}_{safe_muestra}_{today.strftime('%Y%m%d')}.xlsx"
        ...
        if not download:
            return GeFinoSaveResponse(
                ...
                absorcion_pct=ensayo_guardado.absorcion_pct,
        ...
        headers = {
            ...
            "X-Ge-Fino-Id": str(ensayo_guardado.id),
        }
        if storage_object_key:
        ...
        db.rollback()
        logger.exception("Error inesperado en generar_excel_ge_fino")
        raise HTTPException(status_code=500, detail=f"Error generando Excel GE Fino: {str(exc)}")


@router.get("/{ensayo_id}/excel")
def descargar_excel_ge_fino(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    return generar_excel_ge_fino(payload=None, download=True, ensayo_id=ensayo_id, db=db)
