"""
Router for Contenido de Humedad de agregados - ASTM C566-25.
"""
...
from .schemas import (
    ...
    ContHumedadEnsayoResponse,
    ContHumedadRequest,
    ContHumedadSaveResponse,
)

router = APIRouter(prefix="/api/cont-humedad", tags=["Laboratorio Contenido Humedad"])
logger = logging.getLogger(__name__)
_PAYLOAD_COLUMN_READY = False
BUCKET_NAME = "cont-humedad"
...
def _upload_to_supabase_storage(file_bytes: bytes, bucket: str, object_path: str) -> str | None:
    ...
    if not supabase_url or not supabase_key:
        logger.warning("Supabase URL/Key no configurado. Se omite subida de Contenido Humedad.")
        return None
    ...
    try:
        ...
        logger.error("Storage upload Contenido Humedad failed: %s - %s", resp.status_code, resp.text)
        return None
    except Exception as exc:
        logger.error("Error subiendo Contenido Humedad a storage: %s", exc)
        return None
...
def _delete_from_supabase_storage(bucket: str, object_path: str) -> bool:
    ...
    try:
        ...
        if resp.status_code in (200, 204):
            return True
        logger.warning(
            "No se pudo eliminar objeto previo de Contenido Humedad: %s/%s (%s)",
            bucket,
            object_path,
            resp.status_code,
        )
        ...
        logger.warning("Error eliminando objeto previo de Contenido Humedad: %s", exc)
        return False
...
def _move_to_supabase_trash(bucket: str, object_path: str) -> str | None:
    ...
    try:
        ...
        logger.warning("Error moviendo Contenido Humedad a trash con move API: %s", exc)
    ...
    try:
        ...
        logger.warning("Error en fallback copy+delete para trash Contenido Humedad: %s", exc)
        return None
...
def _ensure_payload_column(db: Session) -> None:
    ...
    if is_sqlite:
        for col_def in [
            ...
            "contenido_humedad_pct DOUBLE PRECISION",
        ]:
            try:
                db.execute(text(f"ALTER TABLE cont_humedad_ensayos ADD COLUMN {col_def}"))
            except Exception:
                pass
    else:
        db.execute(text("ALTER TABLE cont_humedad_ensayos ADD COLUMN IF NOT EXISTS payload_json JSON"))
        db.execute(text("ALTER TABLE cont_humedad_ensayos ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ"))
        db.execute(
            text(
                "ALTER TABLE cont_humedad_ensayos "
                "ADD COLUMN IF NOT EXISTS contenido_humedad_pct DOUBLE PRECISION"
            )
        )
    db.flush()
...
def _is_payload_completo(payload: ContHumedadRequest) -> bool:
    required_text_fields = [
        ...
        payload.tamano_maximo_muestra_visual_in,
    ]
    ...
    required_numbers = [
        ...
        payload.masa_recipiente_muestra_seco_g,
        payload.masa_recipiente_muestra_seco_constante_g,
        payload.masa_recipiente_g,
        payload.contenido_humedad_pct,
    ]
    return all(value is not None for value in required_numbers)


def _build_numero_ensayo(payload: ContHumedadRequest) -> str:
    return f"{payload.numero_ot}-CONT-HUM"
...
def _apply_footer_defaults(payload: ContHumedadRequest) -> None:
...
def _guardar_ensayo(
    ...
    if ensayo_id is not None:
        ensayo = (
            db.query(ContHumedadEnsayo)
            .filter(
                ContHumedadEnsayo.id == ensayo_id,
                ContHumedadEnsayo.deleted_at.is_(None),
            )
            .first()
        )
        if not ensayo:
            raise HTTPException(status_code=404, detail="Ensayo Contenido Humedad no encontrado para edicion.")
        old_bucket = ensayo.bucket
        old_object_key = ensayo.object_key
    else:
        ensayo = ContHumedadEnsayo()
        db.add(ensayo)
    ...
    ensayo.contenido_humedad_pct = payload.contenido_humedad_pct
...
def _to_detalle_response(ensayo: ContHumedadEnsayo) -> ContHumedadDetalleResponse:
    payload = None
...
async def listar_ensayos_cont_humedad(
    ...
    return (
        db.query(ContHumedadEnsayo)
        .filter(ContHumedadEnsayo.deleted_at.is_(None))
        .order_by(desc(ContHumedadEnsayo.fecha_creacion))
        .offset(skip)
        .limit(limit)
        .all()
    )
...
@router.get("/{ensayo_id}", response_model=ContHumedadDetalleResponse)
async def obtener_ensayo_cont_humedad(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    _ensure_payload_column(db)
    ...
    if not ensayo:
        raise HTTPException(status_code=404, detail="Ensayo Contenido Humedad no encontrado.")
    return _to_detalle_response(ensayo)


@router.delete("/{ensayo_id}")
async def eliminar_ensayo_cont_humedad(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    _ensure_payload_column(db)
    ...
    try:
        ...
        db.rollback()
        logger.exception("Error enviando ensayo Contenido Humedad a papelera id=%s", ensayo_id)
        raise HTTPException(status_code=500, detail="No se pudo enviar el ensayo Contenido Humedad a papelera.")

    return {
        "message": "Ensayo Contenido Humedad movido a papelera correctamente",
        "id": ensayo_id,
...
@router.post("/excel")
def generar_excel_cont_humedad(
    ...
    try:
        ...
        payload_obj = resolve_export_payload(
            ...
            ensayo_id=ensayo_id,
            db=db,
            model=ContHumedadEnsayo,
            request_model=ContHumedadRequest,
            display_name="Contenido Humedad",
        )

        _apply_footer_defaults(payload_obj)
        excel_bytes = generate_cont_humedad_excel(payload_obj)

        today = date.today()
        filename = build_formato_filename(payload_obj.muestra, "SU20", "CONT. HUMEDAD", template_filename=TEMPLATE_FILENAME)

        safe_ot = _safe_filename(payload_obj.numero_ot, extension="")
        safe_muestra = _safe_filename(payload_obj.muestra, extension="")
        storage_name = f"CONT_HUM_{safe_ot}_{safe_muestra}_{today.strftime('%Y%m%d')}.xlsx"
        ...
        if not download:
            return ContHumedadSaveResponse(
                ...
                contenido_humedad_pct=ensayo_guardado.contenido_humedad_pct,
        ...
        headers = {
            ...
            "X-Cont-Humedad-Id": str(ensayo_guardado.id),
        }
        if storage_object_key:
        ...
        db.rollback()
        logger.exception("Error inesperado en generar_excel_cont_humedad")
        raise HTTPException(status_code=500, detail=f"Error generando Excel Contenido Humedad: {str(exc)}")


@router.get("/{ensayo_id}/excel")
def descargar_excel_cont_humedad(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    return generar_excel_cont_humedad(payload=None, download=True, ensayo_id=ensayo_id, db=db)
