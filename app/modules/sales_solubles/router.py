...
from .schemas import (
    SalesSolublesDetalleResponse,
    SalesSolublesEnsayoResponse,
    SalesSolublesRequest,
    SalesSolublesSaveResponse,
)

router = APIRouter(prefix="/api/sales-solubles", tags=["Laboratorio Sales Solubles"])
logger = logging.getLogger(__name__)
_PAYLOAD_COLUMN_READY = False
BUCKET_NAME = "sales-solubles"
...
def _upload_to_supabase_storage(file_bytes: bytes, bucket: str, object_path: str) -> str | None:
    ...
    if not supabase_url or not supabase_key:
        logger.warning("Supabase URL/Key missing. Upload skipped for Sales Solubles.")
        return None
    ...
    try:
        ...
        logger.error("Storage upload Sales Solubles failed: %s - %s", resp.status_code, resp.text)
        return None
    except Exception as exc:
        logger.error("Error uploading Sales Solubles to storage: %s", exc)
        return None
...
def _delete_from_supabase_storage(bucket: str, object_path: str) -> bool:
    ...
    try:
        ...
        if resp.status_code in (200, 204):
            return True
        logger.warning(
            "Unable to delete Sales Solubles object: %s/%s (%s)",
            bucket,
            object_path,
            resp.status_code,
        )
        ...
        logger.warning("Error deleting Sales Solubles object: %s", exc)
        return False
...
def _move_to_supabase_trash(bucket: str, object_path: str) -> str | None:
    ...
    try:
        ...
        logger.warning("Error moving Sales Solubles to trash via move API: %s", exc)
    ...
    try:
        ...
        logger.warning("Error in fallback copy+delete for Sales Solubles trash: %s", exc)
        return None
...
def _ensure_payload_column(db: Session) -> None:
    ...
    db.execute(text("ALTER TABLE sales_solubles_ensayos ADD COLUMN IF NOT EXISTS payload_json JSON"))
    db.execute(text("ALTER TABLE sales_solubles_ensayos ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ"))
    db.flush()
...
def _is_payload_completo(payload: SalesSolublesRequest) -> bool:
...
def _build_numero_ensayo(payload: SalesSolublesRequest) -> str:
    return f"{payload.numero_ot}-SS"
...
def _apply_footer_defaults(payload: SalesSolublesRequest) -> None:
...
def _guardar_ensayo(
    ...
    if ensayo_id is not None:
        ensayo = (
            db.query(SalesSolublesEnsayo)
            .filter(
                SalesSolublesEnsayo.id == ensayo_id,
                SalesSolublesEnsayo.deleted_at.is_(None),
            )
            .first()
        )
        if not ensayo:
            raise HTTPException(status_code=404, detail="Ensayo Sales Solubles no encontrado para edicion.")
        old_bucket = ensayo.bucket
        old_object_key = ensayo.object_key
    else:
        ensayo = SalesSolublesEnsayo()
        db.add(ensayo)
...
def _to_detalle_response(ensayo: SalesSolublesEnsayo) -> SalesSolublesDetalleResponse:
    payload = None
    if ensayo.payload_json:
        try:
            payload = SalesSolublesRequest.model_validate(ensayo.payload_json)
        except Exception:
            logger.warning("payload_json invalid in sales_solubles_ensayos.id=%s", ensayo.id, exc_info=True)

    return SalesSolublesDetalleResponse(
        id=ensayo.id,
...
async def listar_ensayos(
    ...
    return (
        db.query(SalesSolublesEnsayo)
        .filter(SalesSolublesEnsayo.deleted_at.is_(None))
        .order_by(desc(SalesSolublesEnsayo.fecha_creacion))
        .offset(skip)
        .limit(limit)
        .all()
    )
...
@router.get("/{ensayo_id}", response_model=SalesSolublesDetalleResponse)
async def obtener_ensayo(
    ...
    if not ensayo:
        raise HTTPException(status_code=404, detail="Ensayo Sales Solubles no encontrado.")
...
async def eliminar_ensayo(
    ...
    try:
        ...
        db.rollback()
        logger.exception("Error moving Sales Solubles ensayo to trash id=%s", ensayo_id)
        raise HTTPException(status_code=500, detail="Unable to move Sales Solubles ensayo to trash.")

    return {
        "message": "Ensayo Sales Solubles moved to trash",
        "id": ensayo_id,
...
def generar_excel(
    ...
    try:
        ...
        payload_obj = resolve_export_payload(
            ...
            ensayo_id=ensayo_id,
            db=db,
            model=SalesSolublesEnsayo,
            request_model=SalesSolublesRequest,
            display_name="Sales Solubles",
        )

        _apply_footer_defaults(payload_obj)
        excel_bytes = generate_sales_solubles_excel(payload_obj)

        today = date.today()
        filename = build_formato_filename(payload_obj.muestra, "SU13", "SALES SOLUBLES", template_filename=TEMPLATE_FILENAME)

        safe_ot = _safe_filename(payload_obj.numero_ot, extension="")
        safe_muestra = _safe_filename(payload_obj.muestra, extension="")
        storage_name = f"SS_{safe_ot}_{safe_muestra}_{today.strftime('%Y%m%d')}.xlsx"
        ...
        if not download:
            return SalesSolublesSaveResponse(
        ...
        headers = {
            ...
            "X-SS-Id": str(ensayo_guardado.id),
        }
        if storage_object_key:
        ...
        logger.error("Template missing for Sales Solubles: %s", exc)
        raise HTTPException(status_code=404, detail="Template not found: Template_SALES_SOLUBLES.xlsx")
...
def descargar_excel_sales_solubles(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
