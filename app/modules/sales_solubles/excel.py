"""Excel generator for Sales Solubles (ZIP/XML strategy)."""
...
from .schemas import SalesSolublesCapsula, SalesSolublesRequest
...
def _normalize_text_list(values: Iterable[Any] | None, length: int) -> list[str | None]:
...
def _fill_sheet(sheet_xml: bytes, payload: SalesSolublesRequest) -> bytes:
    ...
    capsulas = list(payload.capsulas or [])
    while len(capsulas) < 2:
        capsulas.append(SalesSolublesCapsula())
    ...
    for idx, refs in enumerate(capsula_refs):
        capsula = capsulas[idx] if idx < len(capsulas) else None
        if capsula is None:
            continue
    ...
    variacion_1 = _normalize_number_list(data.get("peso_constante_variacion_1"), 4)
    peso_2 = _normalize_number_list(data.get("peso_constante_peso_2"), 4)
    variacion_2 = _normalize_number_list(data.get("peso_constante_variacion_2"), 4)
    ...
    _set_cell(sheet_data, "G43", payload.equipo_horno_codigo or "", merge_anchor_map=merge_anchor_map)
    _set_cell(sheet_data, "G44", payload.equipo_balanza_0001_codigo or "", merge_anchor_map=merge_anchor_map)
    _set_cell(sheet_data, "G45", payload.equipo_balanza_001_codigo or "", merge_anchor_map=merge_anchor_map)

    return etree.tostring(root, xml_declaration=True, encoding="UTF-8", standalone=True)


def _fill_drawing(drawing_xml: bytes, payload: SalesSolublesRequest) -> bytes:
    secado_aire = (payload.model_dump(mode="json").get("condicion_secado_aire") or "").strip()
    secado_horno = (payload.model_dump(mode="json").get("condicion_secado_horno") or "").strip()
...
def generate_sales_solubles_excel(payload: SalesSolublesRequest) -> bytes:
    ...
    template_bytes = _get_template_bytes("Template_SALES_SOLUBLES.xlsx")
