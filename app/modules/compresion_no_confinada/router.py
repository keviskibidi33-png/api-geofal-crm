...
"""Router for Compresion No Confinada."""
...
from .schemas import CompresionNoConfinadaDetalleResponse, CompresionNoConfinadaEnsayoResponse, CompresionNoConfinadaRequest, CompresionNoConfinadaSaveResponse

router = APIRouter(prefix="/api/compresion-no-confinada", tags=["Laboratorio Compresion No Confinada"])
logger = logging.getLogger(__name__)
_PAYLOAD_COLUMN_READY = False
BUCKET_NAME = "compresion-no-confinada"
...
def _upload_to_supabase_storage(file_bytes: bytes, bucket: str, object_path: str) -> str | None:
    ...
    if not supabase_url or not supabase_key:
        logger.warning("Supabase URL/Key missing. Upload skipped for Compresion No Confinada.")
        return None
    ...
    try:
        ...
        logger.error("Storage upload Compresion No Confinada failed: %s - %s", resp.status_code, resp.text)
        return None
    except Exception as exc:
        logger.error("Error uploading Compresion No Confinada to storage: %s", exc)
        return None
...
def _delete_from_supabase_storage(bucket: str, object_path: str) -> bool:
    ...
    try:
        ...
        if resp.status_code in (200, 204):
            return True
        logger.warning(
            "Unable to delete Compresion No Confinada object: %s/%s (%s)",
            bucket,
            object_path,
            resp.status_code,
        )
        ...
        logger.warning("Error deleting Compresion No Confinada object: %s", exc)
        return False
...
def _move_to_supabase_trash(bucket: str, object_path: str) -> str | None:
    ...
    try:
        ...
        logger.warning("Error moving Compresion No Confinada to trash via move API: %s", exc)
    ...
    try:
        ...
        logger.warning("Error in fallback copy+delete for Compresion No Confinada trash: %s", exc)
        return None
...
def _ensure_payload_column(db: Session) -> None:
    ...
    db.execute(text("ALTER TABLE compresion_no_confinada_ensayos ADD COLUMN IF NOT EXISTS payload_json JSON"))
    db.execute(text("ALTER TABLE compresion_no_confinada_ensayos ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ"))
    db.flush()
...
def _is_payload_completo(payload: CompresionNoConfinadaRequest) -> bool:
...
def _build_numero_ensayo(payload: CompresionNoConfinadaRequest) -> str:
    return f"{payload.numero_ot}-CNC"
...
def _apply_footer_defaults(payload: CompresionNoConfinadaRequest) -> None:
...
def _guardar_ensayo(
    ...
    if ensayo_id is not None:
        ensayo = (
            db.query(CompresionNoConfinadaEnsayo)
            .filter(
                CompresionNoConfinadaEnsayo.id == ensayo_id,
                CompresionNoConfinadaEnsayo.deleted_at.is_(None),
            )
            .first()
        )
        if not ensayo:
            raise HTTPException(status_code=404, detail="Ensayo Compresion No Confinada no encontrado para edicion.")
        old_bucket = ensayo.bucket
        old_object_key = ensayo.object_key
    else:
        ensayo = CompresionNoConfinadaEnsayo()
        db.add(ensayo)
...
def _to_detalle_response(ensayo: CompresionNoConfinadaEnsayo) -> CompresionNoConfinadaDetalleResponse:
    payload = None
    if ensayo.payload_json:
        try:
            payload = CompresionNoConfinadaRequest.model_validate(ensayo.payload_json)
        except Exception:
            logger.warning("payload_json invalid in compresion_no_confinada_ensayos.id=%s", ensayo.id, exc_info=True)

    return CompresionNoConfinadaDetalleResponse(
        id=ensayo.id,
...
@router.get("/", response_model=list[CompresionNoConfinadaEnsayoResponse])
async def listar_ensayos(
    ...
    return (
        db.query(CompresionNoConfinadaEnsayo)
        .filter(CompresionNoConfinadaEnsayo.deleted_at.is_(None))
        .order_by(desc(CompresionNoConfinadaEnsayo.fecha_creacion))
        .offset(skip)
        .limit(limit)
        .all()
    )
...
@router.get("/{ensayo_id}", response_model=CompresionNoConfinadaDetalleResponse)
async def obtener_ensayo(
    ...
    if not ensayo:
        raise HTTPException(status_code=404, detail="Ensayo Compresion No Confinada no encontrado.")
...
async def eliminar_ensayo(
    ...
    try:
        ...
        db.rollback()
        logger.exception("Error moving Compresion No Confinada ensayo to trash id=%s", ensayo_id)
        raise HTTPException(status_code=500, detail="Unable to move Compresion No Confinada ensayo to trash.")

    return {
        "message": "Ensayo Compresion No Confinada moved to trash",
        "id": ensayo_id,
...
async def generar_excel(
    ...
    try:
        ...
        today = date.today()
        filename = f"CNC_{payload.numero_ot}_{today.strftime('%Y%m%d')}.xlsx"
        ...
        if not download:
            return CompresionNoConfinadaSaveResponse(
        ...
        headers = {
            ...
            "X-CNC-Id": str(ensayo_guardado.id),
        }
        if storage_object_key:
