"""
Router for Granulometría de Agregados - ASTM C136/C136M-25.
"""
...
from .schemas import (
    GranAgregadoDetalleResponse,
    GranAgregadoEnsayoResponse,
    GranAgregadoRequest,
    GranAgregadoSaveResponse,
)

router = APIRouter(prefix="/api/gran-agregado", tags=["Laboratorio Gran Agregado"])
logger = logging.getLogger(__name__)
_PAYLOAD_COLUMN_READY = False
BUCKET_NAME = "gran-agregado"
...
def _upload_to_supabase_storage(file_bytes: bytes, bucket: str, object_path: str) -> str | None:
    ...
    if not supabase_url or not supabase_key:
        logger.warning("Supabase URL/Key no configurado. Se omite subida de Gran Agregado.")
        return None
    ...
    try:
        ...
        logger.error("Storage upload Gran Agregado failed: %s - %s", resp.status_code, resp.text)
        return None
    except Exception as exc:
        logger.error("Error subiendo Gran Agregado a storage: %s", exc)
        return None
...
def _delete_from_supabase_storage(bucket: str, object_path: str) -> bool:
    ...
    try:
        ...
        if resp.status_code in (200, 204):
            return True
        logger.warning(
            "No se pudo eliminar objeto previo de Gran Agregado: %s/%s (%s)",
            bucket,
            object_path,
            resp.status_code,
        )
        ...
        logger.warning("Error eliminando objeto previo de Gran Agregado: %s", exc)
        return False
...
def _move_to_supabase_trash(bucket: str, object_path: str) -> str | None:
    ...
    try:
        ...
        logger.warning("Error moviendo Gran Agregado a trash con move API: %s", exc)
    ...
    try:
        ...
        logger.warning("Error en fallback copy+delete para trash Gran Agregado: %s", exc)
        return None
...
def _ensure_payload_column(db: Session) -> None:
    ...
    db.execute(text("ALTER TABLE gran_agregado_ensayos ADD COLUMN IF NOT EXISTS payload_json JSON"))
    db.execute(text("ALTER TABLE gran_agregado_ensayos ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ"))
    db.execute(text("ALTER TABLE gran_agregado_ensayos ADD COLUMN IF NOT EXISTS error_tamizado_pct DOUBLE PRECISION"))
    db.flush()
...
def _is_payload_completo(payload: GranAgregadoRequest) -> bool:
    ...
    if not any(value is not None for value in payload.masa_retenida_tamiz_g):
        return False

    return payload.masa_antes_tamizado_g is not None and payload.masa_despues_tamizado_g is not None


def _build_numero_ensayo(payload: GranAgregadoRequest) -> str:
    return f"{payload.numero_ot}-GRAN-AGREGADO"
...
def _apply_footer_defaults(payload: GranAgregadoRequest) -> None:
...
def _guardar_ensayo(
    ...
    if ensayo_id is not None:
        ensayo = (
            db.query(GranAgregadoEnsayo)
            .filter(
                GranAgregadoEnsayo.id == ensayo_id,
                GranAgregadoEnsayo.deleted_at.is_(None),
            )
            .first()
        )
        if not ensayo:
            raise HTTPException(status_code=404, detail="Ensayo Gran Agregado no encontrado para edicion.")
        old_bucket = ensayo.bucket
        old_object_key = ensayo.object_key
    else:
        ensayo = GranAgregadoEnsayo()
        db.add(ensayo)
    ...
    ensayo.error_tamizado_pct = payload.error_tamizado_pct
...
def _to_detalle_response(ensayo: GranAgregadoEnsayo) -> GranAgregadoDetalleResponse:
    payload = None
    if ensayo.payload_json:
        try:
            payload = GranAgregadoRequest.model_validate(ensayo.payload_json)
        except Exception:
            logger.warning("payload_json invalido en gran_agregado_ensayos.id=%s", ensayo.id, exc_info=True)

    return GranAgregadoDetalleResponse(
        id=ensayo.id,
        ...
        error_tamizado_pct=ensayo.error_tamizado_pct,
...
async def listar_ensayos_gran_agregado(
    ...
    return (
        db.query(GranAgregadoEnsayo)
        .filter(GranAgregadoEnsayo.deleted_at.is_(None))
        .order_by(desc(GranAgregadoEnsayo.fecha_creacion))
        .offset(skip)
        .limit(limit)
        .all()
    )
...
@router.get("/{ensayo_id}", response_model=GranAgregadoDetalleResponse)
async def obtener_ensayo_gran_agregado(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    _ensure_payload_column(db)
    ...
    if not ensayo:
        raise HTTPException(status_code=404, detail="Ensayo Gran Agregado no encontrado.")
    return _to_detalle_response(ensayo)


@router.delete("/{ensayo_id}")
async def eliminar_ensayo_gran_agregado(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    _ensure_payload_column(db)
...
@router.post("/excel")
async def generar_excel_gran_agregado(
    payload: GranAgregadoRequest,
    ...
    try:
        ...
        if not download:
            return GranAgregadoSaveResponse(
                ...
                error_tamizado_pct=ensayo_guardado.error_tamizado_pct,
        ...
        headers = {
            ...
            "X-Gran-Agregado-Id": str(ensayo_guardado.id),
        }
        if storage_object_key:
