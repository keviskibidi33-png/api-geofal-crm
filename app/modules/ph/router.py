...
router = APIRouter(prefix="/api/ph", tags=["Laboratorio PH"])
...
def _upload_to_supabase_storage(file_bytes: bytes, bucket: str, object_path: str) -> str | None:
    ...
    if not supabase_url or not supabase_key:
        logger.warning("Supabase URL/Key missing. Upload skipped for PH.")
        return None
    ...
    try:
        ...
        logger.error("Storage upload PH failed: %s - %s", resp.status_code, resp.text)
        return None
    except Exception as exc:
        logger.error("Error uploading PH to storage: %s", exc)
        return None
...
def _delete_from_supabase_storage(bucket: str, object_path: str) -> bool:
    ...
    try:
        ...
        if resp.status_code in (200, 204):
            return True
        logger.warning(
            "Unable to delete PH object: %s/%s (%s)",
            bucket,
            object_path,
            resp.status_code,
        )
        ...
        logger.warning("Error deleting PH object: %s", exc)
        return False
...
def _move_to_supabase_trash(bucket: str, object_path: str) -> str | None:
    ...
    try:
        ...
        logger.warning("Error moving PH to trash via move API: %s", exc)
    ...
    try:
        ...
        logger.warning("Error in fallback copy+delete for PH trash: %s", exc)
        return None
...
def _ensure_payload_column(db: Session) -> None:
    ...
    db.execute(text("ALTER TABLE ph_ensayos ADD COLUMN IF NOT EXISTS payload_json JSON"))
    db.execute(text("ALTER TABLE ph_ensayos ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ"))
    db.flush()
...
def _is_payload_completo(payload: PHRequest) -> bool:
...
def _build_numero_ensayo(payload: PHRequest) -> str:
    return f"{payload.numero_ot}-PH"
...
def _apply_footer_defaults(payload: PHRequest) -> None:
...
def _guardar_ensayo(
    ...
    if ensayo_id is not None:
        ensayo = (
            db.query(PHEnsayo)
            .filter(
                PHEnsayo.id == ensayo_id,
                PHEnsayo.deleted_at.is_(None),
            )
            .first()
        )
        if not ensayo:
            raise HTTPException(status_code=404, detail="Ensayo PH no encontrado para edicion.")
...
def _to_detalle_response(ensayo: PHEnsayo) -> PHDetalleResponse:
    payload = None
    if ensayo.payload_json:
        try:
            payload = PHRequest.model_validate(ensayo.payload_json)
        except Exception:
            logger.warning("payload_json invalid in ph_ensayos.id=%s", ensayo.id, exc_info=True)

    return PHDetalleResponse(
        id=ensayo.id,
...
@router.get("/", response_model=list[PHEnsayoResponse])
async def listar_ensayos(
    ...
    return (
        db.query(PHEnsayo)
        .filter(PHEnsayo.deleted_at.is_(None))
        .order_by(desc(PHEnsayo.fecha_creacion))
        .offset(skip)
        .limit(limit)
        .all()
    )
...
@router.get("/{ensayo_id}", response_model=PHDetalleResponse)
async def obtener_ensayo(
    ...
    if not ensayo:
        raise HTTPException(status_code=404, detail="Ensayo PH no encontrado.")
...
async def eliminar_ensayo(
    ...
    try:
        ...
        db.rollback()
        logger.exception("Error moving PH ensayo to trash id=%s", ensayo_id)
        raise HTTPException(status_code=500, detail="Unable to move PH ensayo to trash.")

    return {
        "message": "Ensayo PH moved to trash",
        "id": ensayo_id,
...
def generar_excel(
    ...
    try:
        ...
        payload_obj = resolve_export_payload(
            ...
            db=db,
            model=PHEnsayo,
            request_model=PHRequest,
            display_name="PH",
        )

        _apply_footer_defaults(payload_obj)
        excel_bytes = generate_ph_excel(payload_obj)

        today = date.today()
        filename = build_formato_filename(payload_obj.muestra, "SU03", "PH", template_filename=TEMPLATE_FILENAME)

        safe_ot = _safe_filename(payload_obj.numero_ot, extension="")
        safe_muestra = _safe_filename(payload_obj.muestra, extension="")
        storage_name = f"PH_{safe_ot}_{safe_muestra}_{today.strftime('%Y%m%d')}.xlsx"
        ...
        headers = {
            ...
            "X-PH-Id": str(ensayo_guardado.id),
        }
        if storage_object_key:
        ...
        logger.error("Template missing for PH: %s", exc)
        raise HTTPException(status_code=404, detail="Template not found: Template_PH.xlsx")
    except HTTPException:
        raise
    except Exception as exc:
        logger.exception("Excel export failed for PH")
        raise HTTPException(status_code=500, detail=f"Export failed: {exc}")


@router.get("/{ensayo_id}/excel")
def descargar_excel_ph(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
