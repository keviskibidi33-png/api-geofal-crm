...
from .excel import TEMPLATE_FILENAME, generate_tamiz_excel
from .models import TamizEnsayo
from .schemas import (
    TamizDetalleResponse,
    TamizEnsayoResponse,
    TamizRequest,
    TamizSaveResponse,
)

router = APIRouter(prefix="/api/tamiz", tags=["Laboratorio Tamiz"])
...
def _upload_to_supabase_storage(file_bytes: bytes, bucket: str, object_path: str) -> str | None:
    ...
    if not supabase_url or not supabase_key:
        logger.warning("Supabase URL/Key no configurado. Se omite subida de Tamiz.")
        return None
    ...
    try:
        ...
        logger.error("Storage upload Tamiz failed: %s - %s", resp.status_code, resp.text)
        return None
    except Exception as exc:
        logger.error("Error subiendo Tamiz a storage: %s", exc)
        return None
...
def _delete_from_supabase_storage(bucket: str, object_path: str) -> bool:
    ...
    try:
        ...
        if resp.status_code in (200, 204):
            return True
        logger.warning(
            "No se pudo eliminar objeto previo de Tamiz: %s/%s (%s)",
            bucket,
            object_path,
            resp.status_code,
        )
        ...
        logger.warning("Error eliminando objeto previo de Tamiz: %s", exc)
        return False
...
def _move_to_supabase_trash(bucket: str, object_path: str) -> str | None:
    ...
    try:
        ...
        logger.warning("Error moviendo Tamiz a trash con move API: %s", exc)
    ...
    try:
        ...
        logger.warning("Error en fallback copy+delete para trash Tamiz: %s", exc)
        return None
...
def _is_payload_completo(payload: TamizRequest) -> bool:
    ...
    if payload.procedimiento in {None, "-", ""}:
        return False

    if payload.a_masa_recipiente_g is None or payload.c_masa_recipiente_muestra_seca_constante_g is None:
        return False

    return payload.h_porcentaje_material_fino_pct is not None


def _build_numero_ensayo(payload: TamizRequest) -> str:
    return f"{payload.numero_ot}-TAMIZ"
...
def _apply_footer_defaults(payload: TamizRequest) -> None:
...
def _guardar_ensayo(
    ...
    if ensayo_id is not None:
        ensayo = (
            db.query(TamizEnsayo)
            .filter(
                TamizEnsayo.id == ensayo_id,
                TamizEnsayo.deleted_at.is_(None),
            )
            .first()
        )
        if not ensayo:
            raise HTTPException(status_code=404, detail="Ensayo Tamiz no encontrado para edicion.")
    ...
    ensayo.porcentaje_material_fino_pct = payload.h_porcentaje_material_fino_pct
...
def _to_detalle_response(ensayo: TamizEnsayo) -> TamizDetalleResponse:
    payload = None
    ...
    return TamizDetalleResponse(
        ...
        porcentaje_material_fino_pct=ensayo.porcentaje_material_fino_pct,
...
async def listar_ensayos_tamiz(
    ...
    return (
        db.query(TamizEnsayo)
        .filter(TamizEnsayo.deleted_at.is_(None))
        .order_by(desc(TamizEnsayo.fecha_creacion))
        .offset(skip)
        .limit(limit)
        .all()
    )
...
@router.get("/{ensayo_id}", response_model=TamizDetalleResponse)
async def obtener_ensayo_tamiz(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    _ensure_payload_column(db)
    ...
    if not ensayo:
        raise HTTPException(status_code=404, detail="Ensayo Tamiz no encontrado.")
    return _to_detalle_response(ensayo)


@router.delete("/{ensayo_id}")
async def eliminar_ensayo_tamiz(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    _ensure_payload_column(db)
    ...
    try:
        ...
        db.rollback()
        logger.exception("Error enviando ensayo Tamiz a papelera id=%s", ensayo_id)
        raise HTTPException(status_code=500, detail="No se pudo enviar el ensayo Tamiz a papelera.")

    return {
        "message": "Ensayo Tamiz movido a papelera correctamente",
        "id": ensayo_id,
...
def generar_excel_tamiz(
    ...
    try:
        ...
        payload_obj = resolve_export_payload(
            ...
            db=db,
            model=TamizEnsayo,
            request_model=TamizRequest,
            display_name="Tamiz",
        )

        _apply_footer_defaults(payload_obj)
        excel_bytes = generate_tamiz_excel(payload_obj)

        today = date.today()
        filename = build_formato_filename(payload_obj.muestra, "AG23", "TAMIZ", template_filename=TEMPLATE_FILENAME)

        safe_ot = _safe_filename(payload_obj.numero_ot, extension="")
        safe_muestra = _safe_filename(payload_obj.muestra, extension="")
        storage_name = f"TAMIZ_{safe_ot}_{safe_muestra}_{today.strftime('%Y%m%d')}.xlsx"
        ...
        if not download:
            return TamizSaveResponse(
                ...
                porcentaje_material_fino_pct=ensayo_guardado.porcentaje_material_fino_pct,
        ...
        headers = {
            ...
            "X-Tamiz-Id": str(ensayo_guardado.id),
        }
        if storage_object_key:
        ...
        db.rollback()
        logger.exception("Error inesperado en generar_excel_tamiz")
        raise HTTPException(status_code=500, detail=f"Error generando Excel Tamiz: {str(exc)}")


@router.get("/{ensayo_id}/excel")
def descargar_excel_tamiz(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    return generar_excel_tamiz(payload=None, download=True, ensayo_id=ensayo_id, db=db)
