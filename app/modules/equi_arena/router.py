"""
Router for Equivalente de Arena - ASTM D2419-22.
"""
...
from .schemas import (
    EquiArenaDetalleResponse,
    EquiArenaEnsayoResponse,
    EquiArenaRequest,
    EquiArenaSaveResponse,
)

router = APIRouter(prefix="/api/equi-arena", tags=["Laboratorio EquiArena"])
logger = logging.getLogger(__name__)
_PAYLOAD_COLUMN_READY = False
BUCKET_NAME = "equiarena"
...
def _upload_to_supabase_storage(file_bytes: bytes, bucket: str, object_path: str) -> str | None:
    ...
    if not supabase_url or not supabase_key:
        logger.warning("Supabase URL/Key no configurado. Se omite subida de EquiArena.")
        return None
    ...
    try:
        ...
        logger.error("Storage upload EquiArena failed: %s - %s", resp.status_code, resp.text)
        return None
    except Exception as exc:
        logger.error("Error subiendo EquiArena a storage: %s", exc)
        return None
...
def _delete_from_supabase_storage(bucket: str, object_path: str) -> bool:
    ...
    try:
        ...
        if resp.status_code in (200, 204):
            return True
        logger.warning(
            "No se pudo eliminar objeto previo de EquiArena: %s/%s (%s)",
            bucket,
            object_path,
            resp.status_code,
        )
        ...
        logger.warning("Error eliminando objeto previo de EquiArena: %s", exc)
        return False
...
def _move_to_supabase_trash(bucket: str, object_path: str) -> str | None:
    ...
    try:
        ...
        logger.warning("Error moviendo EquiArena a trash con move API: %s", exc)
    ...
    try:
        ...
        logger.warning("Error en fallback copy+delete para trash EquiArena: %s", exc)
        return None
...
def _ensure_payload_column(db: Session) -> None:
    ...
    db.execute(text("ALTER TABLE equi_arena_ensayos ADD COLUMN IF NOT EXISTS payload_json JSON"))
    db.execute(text("ALTER TABLE equi_arena_ensayos ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ"))
    db.execute(text("ALTER TABLE equi_arena_ensayos ADD COLUMN IF NOT EXISTS equivalente_arena_promedio_pct DOUBLE PRECISION"))
    db.flush()
...
def _is_payload_completo(payload: EquiArenaRequest) -> bool:
    ...
    required_selects = [
        ...
        payload.metodo_agitacion,
        payload.preparacion_muestra,
    ]
    if not all(_is_selected(value) for value in required_selects):
        return False

    if not any(value is not None for value in payload.lectura_arcilla_in):
        return False
    if not any(value is not None for value in payload.lectura_arena_in):
        return False

    return payload.equivalente_arena_promedio_pct is not None


def _build_numero_ensayo(payload: EquiArenaRequest) -> str:
    return f"{payload.numero_ot}-EQUI-ARENA"
...
def _apply_footer_defaults(payload: EquiArenaRequest) -> None:
...
def _guardar_ensayo(
    ...
    if ensayo_id is not None:
        ensayo = (
            db.query(EquiArenaEnsayo)
            .filter(
                EquiArenaEnsayo.id == ensayo_id,
                EquiArenaEnsayo.deleted_at.is_(None),
            )
            .first()
        )
        if not ensayo:
            raise HTTPException(status_code=404, detail="Ensayo EquiArena no encontrado para edición.")
        old_bucket = ensayo.bucket
        old_object_key = ensayo.object_key
    else:
        ensayo = EquiArenaEnsayo()
        db.add(ensayo)
    ...
    ensayo.equivalente_arena_promedio_pct = payload.equivalente_arena_promedio_pct
...
def _to_detalle_response(ensayo: EquiArenaEnsayo) -> EquiArenaDetalleResponse:
    payload = None
    if ensayo.payload_json:
        try:
            payload = EquiArenaRequest.model_validate(ensayo.payload_json)
        except Exception:
            logger.warning("payload_json inválido en equi_arena_ensayos.id=%s", ensayo.id, exc_info=True)

    return EquiArenaDetalleResponse(
        id=ensayo.id,
        ...
        equivalente_arena_promedio_pct=ensayo.equivalente_arena_promedio_pct,
...
@router.get("/", response_model=list[EquiArenaEnsayoResponse])
async def listar_ensayos_equi_arena(
    skip: int = 0,
    limit: int = 100,
    ...
    return (
        db.query(EquiArenaEnsayo)
        .filter(EquiArenaEnsayo.deleted_at.is_(None))
        .order_by(desc(EquiArenaEnsayo.fecha_creacion))
        .offset(skip)
        .limit(limit)
        .all()
    )
...
@router.get("/{ensayo_id}", response_model=EquiArenaDetalleResponse)
async def obtener_ensayo_equi_arena(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    _ensure_payload_column(db)
    ...
    if not ensayo:
        raise HTTPException(status_code=404, detail="Ensayo EquiArena no encontrado.")
    return _to_detalle_response(ensayo)


@router.delete("/{ensayo_id}")
async def eliminar_ensayo_equi_arena(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    _ensure_payload_column(db)
...
@router.post("/excel")
async def generar_excel_equi_arena(
    payload: EquiArenaRequest,
    ...
    try:
        ...
        if not download:
            return EquiArenaSaveResponse(
                ...
                equivalente_arena_promedio_pct=ensayo_guardado.equivalente_arena_promedio_pct,
        ...
        headers = {
            ...
            "X-Equi-Arena-Id": str(ensayo_guardado.id),
        }
        if storage_object_key:
