"""
Router for ABRASION de agregado grueso - ASTM C535-16.
"""
...
router = APIRouter(prefix="/api/abra", tags=["Laboratorio ABRASION"])
...
def _upload_to_supabase_storage(file_bytes: bytes, bucket: str, object_path: str) -> str | None:
    ...
    if not supabase_url or not supabase_key:
        logger.warning("Supabase URL/Key no configurado. Se omite subida de ABRA.")
        return None
    ...
    try:
        ...
        logger.error("Storage upload ABRA failed: %s - %s", resp.status_code, resp.text)
        return None
    except Exception as exc:
        logger.error("Error subiendo ABRA a storage: %s", exc)
        return None
...
def _delete_from_supabase_storage(bucket: str, object_path: str) -> bool:
    ...
    try:
        ...
        if resp.status_code in (200, 204):
            return True
        logger.warning(
            "No se pudo eliminar objeto previo de ABRA: %s/%s (%s)",
            bucket,
            object_path,
            resp.status_code,
        )
        ...
        logger.warning("Error eliminando objeto previo de ABRA: %s", exc)
        return False
...
def _move_to_supabase_trash(bucket: str, object_path: str) -> str | None:
    ...
    try:
        ...
        logger.warning("Error moviendo ABRA a trash con move API: %s", exc)
    ...
    try:
        ...
        logger.warning("Error en fallback copy+delete para trash ABRA: %s", exc)
        return None
...
def _ensure_payload_column(db: Session) -> None:
    ...
    db.execute(text("ALTER TABLE abra_ensayos ADD COLUMN IF NOT EXISTS payload_json JSON"))
    db.execute(text("ALTER TABLE abra_ensayos ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ"))
    db.execute(text("ALTER TABLE abra_ensayos ADD COLUMN IF NOT EXISTS desgaste_promedio_pct DOUBLE PRECISION"))
    db.flush()
...
def _is_payload_completo(payload: AbraRequest) -> bool:
    ...
    main_numbers = [
        ...
        payload.masa_muestra_inicial_seca_g,
        payload.masa_muestra_inicial_seca_constante_g,
    ]
    if not all(value is not None for value in main_numbers):
        return False

    has_tamiz = any(value is not None for value in payload.gradacion_1_tamiz_g + payload.gradacion_2_tamiz_g + payload.gradacion_3_tamiz_g)
    return has_tamiz


def _build_numero_ensayo(payload: AbraRequest) -> str:
    return f"{payload.numero_ot}-ABRA"
...
def _apply_footer_defaults(payload: AbraRequest) -> None:
...
def _guardar_ensayo(
    ...
    if ensayo_id is not None:
        ensayo = (
            db.query(AbraEnsayo)
            .filter(
                AbraEnsayo.id == ensayo_id,
                AbraEnsayo.deleted_at.is_(None),
            )
            .first()
        )
        if not ensayo:
            raise HTTPException(status_code=404, detail="Ensayo ABRA no encontrado para edicion.")
    ...
    ensayo.desgaste_promedio_pct = payload.desgaste_promedio()
...
def _to_detalle_response(ensayo: AbraEnsayo) -> AbraDetalleResponse:
    payload = None
    if ensayo.payload_json:
        try:
            payload = AbraRequest.model_validate(ensayo.payload_json)
        except Exception:
            logger.warning("payload_json invalido en abra_ensayos.id=%s", ensayo.id, exc_info=True)

    return AbraDetalleResponse(
        id=ensayo.id,
        ...
        desgaste_promedio_pct=ensayo.desgaste_promedio_pct,
...
async def listar_ensayos_abra(
    ...
    return (
        db.query(AbraEnsayo)
        .filter(AbraEnsayo.deleted_at.is_(None))
        .order_by(desc(AbraEnsayo.fecha_creacion))
        .offset(skip)
        .limit(limit)
        .all()
    )
...
@router.get("/{ensayo_id}", response_model=AbraDetalleResponse)
async def obtener_ensayo_abra(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    _ensure_payload_column(db)
    ...
    if not ensayo:
        raise HTTPException(status_code=404, detail="Ensayo ABRA no encontrado.")
    return _to_detalle_response(ensayo)


@router.delete("/{ensayo_id}")
async def eliminar_ensayo_abra(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    _ensure_payload_column(db)
    ...
    try:
        ...
        db.rollback()
        logger.exception("Error enviando ensayo ABRA a papelera id=%s", ensayo_id)
        raise HTTPException(status_code=500, detail="No se pudo enviar el ensayo ABRA a papelera.")

    return {
        "message": "Ensayo ABRA movido a papelera correctamente",
        "id": ensayo_id,
...
def generar_excel_abra(
    ...
    try:
        ...
        payload_obj = resolve_export_payload(
            ...
            db=db,
            model=AbraEnsayo,
            request_model=AbraRequest,
            display_name="Abra",
        )

        _apply_footer_defaults(payload_obj)
        excel_bytes = generate_abra_excel(payload_obj, db=db)

        today = date.today()
        filename = build_formato_filename(payload_obj.muestra, "AG26", "ABRA", template_filename=TEMPLATE_FILENAME)

        safe_ot = _safe_filename(payload_obj.numero_ot, extension="")
        safe_muestra = _safe_filename(payload_obj.muestra, extension="")
        storage_name = f"ABRA_{safe_ot}_{safe_muestra}_{today.strftime('%Y%m%d')}.xlsx"
        ...
        if not download:
            return AbraSaveResponse(
                ...
                desgaste_promedio_pct=ensayo_guardado.desgaste_promedio_pct,
        ...
        headers = {
            ...
            "X-ABRA-Id": str(ensayo_guardado.id),
        }
        if storage_object_key:
        ...
        db.rollback()
        logger.exception("Error inesperado en generar_excel_abra")
        raise HTTPException(status_code=500, detail=f"Error generando Excel ABRA: {str(exc)}")


@router.get("/{ensayo_id}/excel")
def descargar_excel_abra(
    ensayo_id: int,
    db: Session = Depends(get_db_session),
):
    return generar_excel_abra(payload=None, download=True, ensayo_id=ensayo_id, db=db)
