...
def _get_template_path(template_id: str | None = None) -> Path:
    ...
    filename = TEMPLATE_VARIANTS.get(template_id, 'Template.xlsx') if template_id else 'Template.xlsx'
...
async def get_quote_by_id(quote_id: str):
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            data = dict(row)
            # Ensure condiciones_ids is a clean list (handle list, None, or postgres string "{...}")
...
async def update_quote(quote_id: str, payload: QuoteExportRequest):
    ...
    print(f"DEBUG: Updating quote {quote_id}")
    print(f"DEBUG: Payload plazo_dias={payload.plazo_dias}, condicion_pago={payload.condicion_pago}")
    print(f"DEBUG: Payload condiciones_ids={payload.condiciones_ids}")
    ...
    try:
        ...
        if payload.condiciones_ids and len(payload.condiciones_ids) > 0:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                cur.execute(f"""
                    SELECT texto FROM condiciones_especificas
                    WHERE id = ANY(%s::uuid[]) AND activo = true
                    ORDER BY orden
        ...
        template_path = _get_template_path(payload.template_id)
        xlsx_bytes = export_xlsx_direct(str(template_path), payload_dict)
