...
def dump_sheet_labels(template_path, sheet_rel_path):
    print(f"\n--- Dumping labels from: {sheet_rel_path} ---")
    shared_strings = []
    ...
    with zipfile.ZipFile(template_path, 'r') as z:
        if f"xl/{sheet_rel_path}" not in z.namelist():
            print(f"Sheet {sheet_rel_path} not found")
            return
...
if __name__ == "__main__":
    ...
    if template.exists():
        for i in range(8, 13):
            dump_sheet_labels(template, f"worksheets/sheet{i}.xml")
    else:
