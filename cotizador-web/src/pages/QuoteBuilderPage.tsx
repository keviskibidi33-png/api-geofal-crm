        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[80vh] overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-slate-200">
              <h2 className="text-lg font-semibold text-slate-900">Cargar Plantilla</h2>
              <p className="text-sm text-slate-500">Selecciona una plantilla para aplicar sus configuraciones</p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {loadingPlantillas ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-slate-500 mb-1">No hay plantillas guardadas</p>
                  <p className="text-xs text-slate-400">Crea una cotización y guárdala como plantilla</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {plantillas.map((plantilla) => (
                    <div
                      key={plantilla.id}
                      className="border border-slate-200 rounded-lg p-4 hover:border-blue-400 hover:bg-blue-50 cursor-pointer transition-all"
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <span className="text-xs bg-slate-100 text-slate-600 px-2 py-1 rounded">
                          Usada {plantilla.veces_usada || 0}x
                        </span>
                      </div>
                      <div className="flex items-center gap-4 text-xs text-slate-500">
                        <span className="flex items-center gap-1">
                          <FileText className="h-3 w-3" />
                          {Array.isArray(plantilla.items_json) ? plantilla.items_json.length : JSON.parse(plantilla.items_json || '[]').length} items
                        </span>
                        {plantilla.plazo_dias && (
                          <span>Plazo: {plantilla.plazo_dias} días</span>
                        )}
                        {plantilla.condicion_pago && (
                          <span>Pago: {plantilla.condicion_pago}</span>
                        )}
                      </div>
                    </div>
                  ))}
